...
function MonsterAI( main, x, y, type ) {
  ...
  this.strategy = new AIStrategy( main.pacman, this, main.safetile, main.opposites );
...
MonsterAI.prototype.feelForward = function() {
  this.marker.x = this.main.game.math.snapToFloor(
    Math.floor( this.x ), this.main.gridsize ) / this.main.gridsize;
  this.marker.y = this.main.game.math.snapToFloor(
    Math.floor( this.y ), this.main.gridsize ) / this.main.gridsize;

  // move marker forward (see forward)
  this.forwardMarker = this.getForward(
    new Phaser.Point( this.marker.x, this.marker.y ) );
  ...
  this.directions[ 0 ] = this.main.map.getTile(
    this.forwardMarker.x, this.forwardMarker.y, this.main.layer.index );
  this.directions[ 1 ] = this.main.map.getTileLeft(
    this.main.layer.index, this.forwardMarker.x, this.forwardMarker.y );
  this.directions[ 2 ] = this.main.map.getTileRight(
    this.main.layer.index, this.forwardMarker.x, this.forwardMarker.y );
  this.directions[ 3 ] = this.main.map.getTileAbove(
    this.main.layer.index, this.forwardMarker.x, this.forwardMarker.y );
  this.directions[ 4 ] = this.main.map.getTileBelow(
    this.main.layer.index, this.forwardMarker.x, this.forwardMarker.y );

  // calculate next direction
  this.nextDirection = this.nextDirectionFinder(
    this.directions, this.current, this.strategy );
  ...
  if ( this.debug ) {
    for ( var t = 0; t < 5; t++ ) {
        ...
        this.game.debug.geom( new Phaser.Rectangle(
          this.directions[ t ].worldX, this.directions[ t ].worldY,
         this.gridsize, this.gridsize ), color, true );
    }
    // this.game.debug.geom(new Phaser.Line())
    this.game.debug.geom( new Phaser.Circle(
      this.turnPoint.x, this.turnPoint.y, 5 ), 'rgba(255,0,0,1)' );
    // this.game.debug.geom(new Phaser.Circle(
    // this.futurePoint.x, this.futurePoint.y, 5), 'rgba(0,255,0,1)');
    this.game.debug.geom( new Phaser.Circle(
      this.x, this.y, 5 ), 'rgba(0,0,0,1)' );
    this.game.debug.geom( new Phaser.Circle(
      this.marker.x, this.marker.y, 5 ), 'rgba(0,255,0,1)' );
    // this.game.debug.geom(new Phaser.Rectangle(
    // this.body.x, this.body.y, this.body.width, this.body.height), 0xffffff, true);
